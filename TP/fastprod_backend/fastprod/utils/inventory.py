...
def delete_item_from_hosts_yaml(yaml_file_path="fastprod/inventory/hosts.yaml",
                                save=True,
                                item=None):
    """Delete item from hosts.yaml"""
    with open(yaml_file_path, 'r', encoding='utf8') as yaml_file:
        hosts = yaml.safe_load(yaml_file)
        new_hosts = hosts.copy()
